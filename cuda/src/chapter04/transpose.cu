...
void copyRowHost(float *out, float *in, const int nx, const int ny)
{
...
void copyColHost(float *out, float *in, const int nx, const int ny)
{
    for (auto ix = 0; ix < nx; ++ix)
    {
        for (auto iy = 0; iy < ny; ++iy)
...
// case 8 transpose kernel: read in rows and write in columns, unrolled with pragma
__global__ void transposeNaiveRowUnrollPragma(float *out, float *in, const int nx,
                                  const int ny)
{
    unsigned int ix = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y;
...
// case 9 transpose kernel: read in columns and write in rows, unrolled with pragma
__global__ void transposeNaiveColUnrollPragma(float *out, float *in, const int nx,
                                  const int ny)
{
    int n = 4;
    unsigned int ix = blockDim.x * blockIdx.x * n + threadIdx.x;
    unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y;

    unsigned int ti = iy * nx + ix; // access in rows
    unsigned int to = ix * ny + iy; // access in columns

    if (ix + (n-1) * blockDim.x < nx && iy < ny)
    {
#pragma unroll
        for (int i = 0; i < n; ++i)
        {
            out[to + ny * i * blockDim.x] = in[ti + i * blockDim.x];
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    copyColHost(h_Out, h_A, nx, ny);
    double iElaps = seconds() - iStart;
    printf("copyColHost    elapsed %f sec\n", iElaps);

    checkResult(h_A, h_Out, nx * ny);

    iStart = seconds();
    copyRowHost(h_Out, h_A, nx, ny);
    iElaps = seconds() - iStart;
    printf("copyRowHost         elapsed %f sec\n", iElaps);
    checkResult(h_A, h_Out, nx * ny);
    ...
    case 8:
        kernel = &transposeNaiveRowUnrollPragma;
        kernelName = "transposeNaiveRowUnrollPragma";
        break;

    case 9:
        kernel = &transposeNaiveColUnrollPragma;
        kernelName = "transposeNaiveColUnrollPragma";
        grid.x = (nx + block.x * 4 - 1) / (block.x * 4);
        break;
    }
